...
?>
			<div class="account-main">
		        ...
		        <div class="maraphon_day">
				    ...
				    if ((current_time("j") + 0) >= 29) {
						...
						echo 'С новым 2021 годом! Стартуем 3 января';
					} else {
				...
				<?php //Управление анкетами в "Личные данные" в зависимости от купленного меню
					...
					if (is_user_role('maraphon_1200', $user_id) || is_user_role('maraphon_1400', $user_id) || is_user_role('maraphon_1600', $user_id) || is_user_role('maraphon_1800', $user_id) || is_user_role('maraphon_2000', $user_id) || is_user_role('maraphon_2200', $user_id) ) {
						$show_maraphon_tab = 1;
					};
					
					$women_menu_lk_value = get_the_author_meta( 'women_menu_lk', $current_user->ID );
					
					if (
						$women_menu_lk_value == 'women_menu_1200' || 
						$women_menu_lk_value == 'women_menu_1400' || 
						$women_menu_lk_value == 'women_menu_1600' || 
						$women_menu_lk_value == 'women_menu_1800' ||
						$women_menu_lk_value == 'women_menu_2000' ||
						$women_menu_lk_value == 'women_menu_2200'
					) {
						$show_women_menu_tab = 1;
					};
					
					$men_menu_lk_value = get_the_author_meta( 'men_menu_lk', $current_user->ID );
					
					if (
						$men_menu_lk_value == 'men_menu_1600' || 																												
						$men_menu_lk_value == 'men_menu_1800' || 
						$men_menu_lk_value == 'men_menu_2000' || 
						$men_menu_lk_value == 'men_menu_2200' || 
						$men_menu_lk_value == 'men_menu_2500' 
					) {
						$show_men_menu_tab = 1;
					};
					...
					//управление анкетой для участников марафона
					if (
						get_the_author_meta( 'maraphon_counter', $current_user->ID ) == '0' ||
						get_the_author_meta( 'maraphon_counter', $current_user->ID ) > 0 ||
						(
							is_user_role('notconfirm', $current_user->ID) &&
							(
								substr(get_the_author_meta( 'men_menu_lk', $current_user->ID ),0 , 8) != 'men_menu' ||
								substr(get_the_author_meta( 'women_menu_lk', $current_user->ID ),0 , 10) != 'women_menu'
							)
						)
					)
					{
					...
					//управление анкетой для покупателей женского меню
					$check_women_menu_time = $wpdb->get_var( 
					"
					SELECT 
						orders.time
					...
					//$check_women_menu_time = 1641071766;
										
					if (	
							(is_user_role('administrator', $current_user->ID) || is_user_role('content_1', $user_id)) ||
							(get_the_author_meta( 'women_menu_lk', $current_user->ID ) == 'women_menu') ||
							($check_women_menu_time > $time_now && $show_women_menu_tab == 1)
					   ) {
						$lk_menu_buy_div_flag = 'display:block';
						$lk_table_women_menu_flag = 'display:block';
					};
					
					if ($check_women_menu_time <= $time_now && $show_women_menu_tab == 1 ) {
						update_user_meta($user_id, 'women_menu_lk', 'women_menu');
					};
					
					
					//управление анкетой для покупателей мужского меню
					if (																		
						substr(get_the_author_meta( 'men_menu_lk', $current_user->ID ),0 , 8) == 'men_menu'
					   ) {
						$lk_menu_buy_div_flag = 'display:block';
					...
					$check_men_menu_time = $wpdb->get_var( 
					"
					SELECT 
						orders.time
					FROM wpux_orders orders
					...
					//$check_men_menu_time = 1642071766;
					
					if (
						   ( $check_men_menu_time > $time_now && $show_men_menu_tab == 1 ) ||
						   (
							 is_user_role('administrator', $current_user->ID) ||
							 is_user_role('content_1', $user_id)	
						   ) 
					   ) 
					   {
						$lk_menu_buy_div_flag = 'display:block';
				...
				if (($check_men_menu_time <= $time_now) && $show_men_menu_tab == 1) {
		        ...
		        <script>
					...
					'<style>
						#account-form {display: block !important;}
						#account-result {display: block !important; border-bottom: 10px solid #252525; background-color: #d5d5d5;}
						#tab1 {display: none !important;}
						#account-report a {color: #d5d5d5 !important;}
						#account-report {cursor: default; pointer-events: none;}
				    </style>
				    ';
				    }
					...
					if ( $show_women_menu_tab == 1 && $show_admin_tab != 1 && $show_maraphon_tab != 1 ) {
				 	echo 
					'<style>
						#account-form {display: block !important;}
						#account-result {display: block !important; border-bottom: 10px solid #252525; background-color: #d5d5d5;}
				    ...
				    if ( $show_men_menu_tab == 1 && $show_admin_tab != 1 && $show_maraphon_tab != 1 ) {
				 	echo 
					'<style>
						#account-form {display: block !important;}
						#account-result {display: block !important; border-bottom: 10px solid #252525; background-color: #d5d5d5;}
						...
						#account-report {cursor: default; pointer-events: none;}
						#tab3 {display: block;}
						#account-men-menu {display: block;}
				    </style>';
				    } else if ( $show_men_menu_tab == 1 && $show_maraphon_tab == 1 ) {
					    echo 
					'<style>
						#account-form {display: block !important;}
						#account-result {display: block !important; border-bottom: 10px solid #252525; background-color: #d5d5d5;}
				    ...
				    if ( $show_workout_tab == 1 && $show_admin_tab != 1 && $show_maraphon_tab != 1 ) {
				 	echo 
					'<style>
						#account-form {display: block !important;}
						#account-result {display: block !important; border-bottom: 10px solid #252525; background-color: #d5d5d5;}
			 ...
			 <div class="lk-content">
					...
					{
					echo '<div style="height: 600px; padding-top: 100px"><p style="font-size: 44px; text-align: center;">ДОСТУП К САЙТУ ЗАБЛОКИРОВАН ЗА НАРУШЕНИЕ ПРАВИЛ МАРАФОНА ДО ВЫЯСНЕНИЯ ОБСТОЯТЕЛЬСТВ</p></div>';	
					} else { */
					?>
				...
				<!--	<p style="width: 80%;
						...
						Поле алкоголь обязательно для заполнения<br>Если ответили да, то укажите какой и сколько<br>Если читмил или срыв с алкоголем, ставим обе галочки
					</p> -->
				<?php   global $check_orders;						
				...
				?>                    
					...
					<?php	//Выводим текущий месяц и год
							...
							echo $month; echo '&nbsp'; echo $year;
					?>	 
					</span>
			        ...
			        <?php
						...
						//$check_packet = 'пакет "Семейный"';   																  // убрать 1 февраля 2021
					...
					<form type="post" action="" style="display:<?php echo $form_report_flag; ?>" id="dailyReportForm">
						<div class="confirm_form_block">   
							...
							<h1 id="daily_report_for_user_h1">Введите Ваши данные: </h1>
								...
								<script src="http://maraphon.online/wp-content/themes/maraphon/js/jquery-1.9.1.min.js"></script>
								<script>
	        ...
	        <?php
				...
				is_user_role('administrator', $user_id) ||
				is_user_role('content_1', $user_id)
				) {
			...
			$date_access = date('d.m.Y',$check_women_menu_time);
			
			if ($check_women_menu_time > 0) {
				$time_phrase = 'Ваше меню открыто до '.$date_access.'.';
			} else {
				$time_phrase = 'Ваше меню открыто до 28 '.$menu_month.'.';
				//$time_phrase = 'Ваше меню открыто до 30 декабря.';
			};
			
			if (get_the_author_meta( 'women_menu_lk', $current_user->ID ) == 'women_menu_1200') {
				echo '<div class="menu_download_link">Ваше меню открыто до '.$date_access.'<a href="http://maraphon.online/wp-content/uploads/menu_download/'.get_field('women_menu_1200').'" target="_blank">Скачать все меню</a></div>';
				} else if (get_the_author_meta( 'women_menu_lk', $current_user->ID ) == 'women_menu_1400') {
					echo '<div class="menu_download_link">Ваше меню открыто до '.$date_access.'<a href="http://maraphon.online/wp-content/uploads/menu_download/'.get_field('women_menu_1400').'" target="_blank">Скачать все меню</a></div>';
					} else if (get_the_author_meta( 'women_menu_lk', $current_user->ID ) == 'women_menu_1600') {
						echo '<div class="menu_download_link">Ваше меню открыто до '.$date_access.'<a href="http://maraphon.online/wp-content/uploads/menu_download/'.get_field('women_menu_1600').'" target="_blank">Скачать все меню</a></div>'; 
						} else if (get_the_author_meta( 'women_menu_lk', $current_user->ID ) == 'women_menu_1800') {
							echo '<div class="menu_download_link">Ваше меню открыто до '.$date_access.'<a href="http://maraphon.online/wp-content/uploads/menu_download/'.get_field('women_menu_1800').'" target="_blank">Скачать все меню</a></div>';
							} else if (get_the_author_meta( 'women_menu_lk', $current_user->ID ) == 'women_menu_2000') {
								echo '<div class="menu_download_link">Ваше меню открыто до '.$date_access.'<a href="http://maraphon.online/wp-content/uploads/menu_download/'.get_field('women_menu_2000').'" target="_blank">Скачать все меню</a></div>';
								} else if (get_the_author_meta( 'women_menu_lk', $current_user->ID ) == 'women_menu_2200') {
									echo '<div class="menu_download_link">Ваше меню открыто до '.$date_access.'<a href="http://maraphon.online/wp-content/uploads/menu_download/'.get_field('women_menu_2200').'" target="_blank">Скачать все меню</a></div>';
								}; /* else {
									echo '<div style="box-shadow: 0 10px 18px -13px #000; border-radius: 3px; margin-bottom: 30px; background-color: #f6f6f6; padding-top: 15px; padding-bottom: 5px;">
												<p style="text-align: center; font-size: 26px; margin-bottom: 5px;">'.$time_phrase.' Если вы хотите сохранить (распечатать) меню - услуга платная.</p>
												<p style="text-align: center; font-size: 26px; margin-bottom: 5px;">Стоимость 500 руб.</p>
												<p style="text-align: center; font-size: 26px; margin-bottom: 5px;">При покупке мужского меню, скачивание двух меню 700 руб</p>
												<p style="text-align: center; font-size: 26px; margin-bottom: 10px;"><a href="https://api.whatsapp.com/send?phone=+79095496086" target="_blank" style="color: #1ab02a">Написать в Whatsapp для заказа</a></p>
										  </div>';
								}; */
			?>
			    ...
			    '<style>
				    #account-menu {display: block !important}
					#account-workout {display: block !important}
				    #account-report {display: block !important}
				</style>';
			 	} else if (is_user_role('notconfirm', $user_id)) {
				 	if (get_the_author_meta( 'women_menu_lk', $current_user->ID ) == 'women_menu_1200') {
				 		echo do_shortcode("[pt_view id=13cadb8au2]");
			 			} else 
			 			if (get_the_author_meta( 'women_menu_lk', $current_user->ID ) == 'women_menu_1400') {
			 				echo do_shortcode("[pt_view id=00ed6adpmi]");
			 				'<style>#account-menu {display: block !important; border-right: none}</style>';
						 	} else 
						 	if (get_the_author_meta( 'women_menu_lk', $current_user->ID ) == 'women_menu_1600') {
								echo do_shortcode("[pt_view id=1f263925vx]");
								'<style>#account-menu {display: block !important; border-right: none}</style>';
								} else 
								if (get_the_author_meta( 'women_menu_lk', $current_user->ID ) == 'women_menu_1800') {
									echo do_shortcode("[pt_view id=8da1dd14ae]");	
									'<style>#account-menu {display: block !important; border-right: none}</style>';
									} else 
									if (get_the_author_meta( 'women_menu_lk', $current_user->ID ) == 'women_menu_2000') {
										echo do_shortcode("[pt_view id=0697055rsa]");
										'<style>#account-menu {display: block !important; border-right: none}</style>';
										} else 
										if (get_the_author_meta( 'women_menu_lk', $current_user->ID ) == 'women_menu_2200') {
											echo do_shortcode("[pt_view id=f1d34dcci0]");
											'<style>#account-menu {display: block !important; border-right: none}</style>';
											} else 
											if (get_the_author_meta( 'women_menu_lk', $current_user->ID ) == 'women_menu_2500') {
												echo do_shortcode("[pt_view id=043c990374]");
												'<style>#account-menu {display: block !important; border-right: none}</style>';};
			 	} else 
			...
			<?php //----- МУЖСКОЕ МЕНЮ --------//
			if (																		              //управление анкетой для покупателей мужского меню
					get_the_author_meta( 'men_menu_lk', $current_user->ID ) == 'men_menu_1600' || 
					get_the_author_meta( 'men_menu_lk', $current_user->ID ) == 'men_menu_1800' || 
					get_the_author_meta( 'men_menu_lk', $current_user->ID ) == 'men_menu_2000' || 
					get_the_author_meta( 'men_menu_lk', $current_user->ID ) == 'men_menu_2200' || 
					...
					is_user_role('content_1', $user_id)
						...
						<div class="menu_contain_block">';
							...
							$date_access_men = date('d.m.Y',$check_men_menu_time);						
							
							$member_men_menu = get_user_meta ( $user_id, 'men_menu_lk', true);
						    ...
						    '<style>
						 	...
						 	if (is_user_logged_in() && (is_user_role('maraphon_1200', $user_id) || is_user_role('maraphon_1400', $user_id) || is_user_role('maraphon_1600', $user_id) || is_user_role('maraphon_1800', $user_id) || is_user_role('maraphon_2000', $user_id) || is_user_role('maraphon_2200', $user_id) || is_user_role('maraphon_2500', $user_id) )) {
						 			
						 			if ($member_men_menu == 'men_menu_1800') {
									 	echo '<div class="menu_download_link">Ваше меню открыто до '.$date_access_men.'</div>';
										echo do_shortcode("[pt_view id=7b3a932wgj]");
										} else if ($member_men_menu == 'men_menu_2000') {
											echo '<div class="menu_download_link">Ваше меню открыто до '.$date_access_men.'</div>';
											echo do_shortcode("[pt_view id=45f8e99nuq]");
											} else if ($member_men_menu == 'men_menu_2200') {
												echo '<div class="menu_download_link">Ваше меню открыто до '.$date_access_men.'</div>';
												echo do_shortcode("[pt_view id=b0f7c54u9z]");
												} else if ($member_men_menu == 'men_menu_2500') {
												echo '<div class="menu_download_link">Ваше меню открыто до '.$date_access_men.'</div>';
												echo do_shortcode("[pt_view id=aa0007de0i]");
													} else {
													echo '<div style="height: 500px; background-color: white; padding-top: 150px; text-align: center; font-size: 48px;">Вы не приобрели мужское меню</div>';
													'<style>#account-menu {display: block !important; border-right: none}</style>';};
							} else
							if (is_user_logged_in() && (is_user_role('notconfirm', $user_id))) {
								if ($member_men_menu == 'men_menu_1600') {
								 	echo '<div class="menu_download_link">Ваше меню открыто до '.$date_access_men.'<a href="http://maraphon.online/wp-content/uploads/menu_download/'.get_field('men_menu_1600').'" target="_blank">Скачать все меню</a></div>';
									echo do_shortcode("[pt_view id=0b7470b7qh]");
										} else if ($member_men_menu == 'men_menu_1800') {
									 	echo '<div class="menu_download_link">Ваше меню открыто до '.$date_access_men.'<a href="http://maraphon.online/wp-content/uploads/menu_download/'.get_field('men_menu_1800').'" target="_blank">Скачать все меню</a></div>';
										echo do_shortcode("[pt_view id=8a1e145xym]");
										} else if ($member_men_menu == 'men_menu_2000') {
											echo '<div class="menu_download_link">Ваше меню открыто до '.$date_access_men.'<a href="http://maraphon.online/wp-content/uploads/menu_download/'.get_field('men_menu_2000').'" target="_blank">Скачать все меню</a></div>';
											echo do_shortcode("[pt_view id=f560d0ee9q]");
											} else if ($member_men_menu == 'men_menu_2200') {
												echo '<div class="menu_download_link">Ваше меню открыто до '.$date_access_men.'<a href="http://maraphon.online/wp-content/uploads/menu_download/'.get_field('men_menu_2200').'" target="_blank">Скачать все меню</a></div>';
												echo do_shortcode("[pt_view id=b0e9321mr9]");
												} else if ($member_men_menu == 'men_menu_2500') {
												echo '<div class="menu_download_link">Ваше меню открыто до '.$date_access_men.'<a href="http://maraphon.online/wp-content/uploads/menu_download/'.get_field('men_menu_2500').'" target="_blank">Скачать все меню</a></div>';
												echo do_shortcode("[pt_view id=797c21bu37]");
													} else {
													echo '<div style="height: 500px; background-color: white; padding-top: 150px; text-align: center; font-size: 48px;">Вы не приобрели мужское меню</div>';
													'<style>#account-menu {display: block !important; border-right: none}</style>';};
      ...
      echo '</div>';
			...
			?>
		        ...
		        <div class="menu_contain_block">
	            <?php 
				$member_workout_class = get_user_meta ( $current_user->ID, 'workout_class', true);
			...
			<?php
			global $wpdb;
			$current_user_inst = wp_get_current_user();
			$user_id_inst = $current_user_inst->ID;
			...
			SELECT 
				...
				WHERE users.user_id = $user_id_inst	  
			"
			);
			?> 
			...
			<p class="mobile_user_maraphon_count">Мой результат после 
				<?php 
					...
					$maraphon_counter = get_the_author_meta('maraphon_counter', $current_user->ID);
		...
		<div class="monthly_report_text_block" style="display:<?php echo $form_monthly_report_flag; ?>">
			...
			<div class="lk-maraphon-buy" style="<?php echo $lk_maraphon_buy_div_flag ?>">
                    ...
                    <tr class="form-telephone">
                        ...
                        <td><input class="text-input" name="telephone" type="text" id="telephone" value="<?php the_author_meta( 'telephone', $current_user->ID ); ?>" /></td>
                    </tr>
                    
                    <tr class="form-email">
                        <th><label for="email">E-mail</label></th>
                        <td><input class="text-input" readonly name="email" type="text" id="email" value="<?php the_author_meta( 'user_email', $current_user->ID ); ?>" /></td>
                    </tr>
                    
                    <tr class="form-town">
                        <th><label for="town">Город (нас. пункт)</label></th>
                        <td><input class="text-input" name="town" type="text" id="town" value="<?php the_author_meta( 'town', $current_user->ID ); ?>" /></td>
                    </tr>

                    <tr class="form-textarea">
                        <th><label for="what_you_know">Откуда узнали о марафоне?</label></th>
                        <td><textarea name="what_you_know" id="what_you_know" rows="5" cols="1"><?php the_author_meta( 'what_you_know', $current_user->ID ); ?></textarea></td>
                    </tr>
</table>
                  ...
                  <table class="lk-table-parameters">
                    ...
                    <tr class="form-age">
                        ...
                        <td><input class="text-input" name="age" type="number" id="age" value="<?php the_author_meta( 'age', $current_user->ID ); ?>" /></td>
                    </tr>              
                    
                    <tr class="form-height">
                        <th><label for="height">Рост, до 1см</label></th>
                        <td><input class="text-input" name="height" type="number" id="height" value="<?php the_author_meta( 'height', $current_user->ID ); ?>" /></td>
                    </tr>
                    
                     <tr class="form-weight-at-start">
                        <th><label for="weight-at-start">Текущий вес, до 0,1кг</label></th>
                        <td><input class="text-input" name="weight-at-start" type="number" id="weight-at-start" step="0.1" value="<?php the_author_meta( 'weight-at-start', $current_user->ID ); ?>" /></td>
                    </tr>
                    ...
                    <tr id="daily-activity-row">
                        ...
                        <td>
	                    <?php 
	                       	$daily_activity_value = get_user_meta( $current_user->ID, 'daily-activity', true);
				  	...
				  	<tr class="form-age">
                        ...
                        <td><input class="text-input" name="weight_at_1_maraphon" type="number" id="weight_at_1_maraphon" step="0.1" value="<?php the_author_meta( 'weight_at_1_maraphon', $current_user->ID ); ?>" /></td>
                    </tr> 
                    
                    <tr class="form-dream-weight">
                        <th><label for="dream-weight">Желаемый вес, кг</label></th>
                        <td><input class="text-input" name="dream-weight" type="text" id="dream-weight" step="0.1" value="<?php the_author_meta( 'dream-weight', $current_user->ID ); ?>" /></td>
                    </tr>
                        
                    <tr id="daily-activity-confirm-row" style="display: none;">
                        <th><label for="daily-activity">Коэф. активности</label></th>
                        <td><input class="text-input" name="daily-activity" type="number" id="daily-activity" disabled value="<?php the_author_meta( 'daily-activity', $current_user->ID ); ?>" /></td>
		   ...
		   <h1 class="mobile-form-header"><i class="fa fa-venus"></i>&nbsp;&nbsp;&nbsp;Женская физиология</h1>
                ...
                <tr class="form-pregnant">
                        ...
                        <td><input class="text-input" name="pregnant" type="text" id="pregnant" value="<?php the_author_meta( 'pregnant', $current_user->ID ); ?>" /></td>
                </tr>
                ...
                <tr class="form-breastfeed">
                        ...
                        <td>
                        <?php 
	                       		$breastfeed_value = get_user_meta ( $current_user->ID, 'breastfeed', true);
	                ...
	                <td><input class="text-input" name="first_menstruation_day" type="text" id="first_menstruation_day" value="<?php the_author_meta( 'first_menstruation_day', $current_user->ID ); ?>" /></td>
                	</tr>
                    
                    <tr class="form-textarea">
			                <th><label for="contraceptive">Принимаете ли противозачаточные?</label></th>
			                <td><textarea name="contraceptive" id="contraceptive" rows="5" cols="1"><?php the_author_meta( 'contraceptive', $current_user->ID ); ?></textarea></td>
					</tr>
					...
					<tr class="form-textarea">
						...
						<td><textarea name="hormonal-background" id="hormonal-background" rows="5" cols="1"><?php the_author_meta( 'hormonal-background', $current_user->ID ); ?></textarea></td>
					</tr>

		            <tr class="form-textarea">
		                <th><label for="hair-problems">Есть ли проблемы с волосами? Какие?</label></th>
			            <td><textarea name="hair-problems" id="hair-problems" rows="5" cols="1"><?php the_author_meta( 'hair-problems', $current_user->ID ); ?></textarea></td>
		            </tr>
		
		            <tr class="form-textarea">
					    <th><label for="intestin_problems">Есть ли проблемы с кишечником?</label></th>
					    <td><textarea name="intestin_problems" id="intestin_problems" rows="5" cols="1"><?php the_author_meta( 'intestin_problems', $current_user->ID ); ?></textarea></td>
		            </tr>
		            
		            <tr class="form-textarea">
					    <th><label for="joint_problems">Есть ли проблемы с суставами? Травма?</label></th>
					    <td><textarea name="joint_problems" id="joint_problems" rows="5" cols="1"><?php the_author_meta( 'joint_problems', $current_user->ID ); ?></textarea></td>
		            </tr>
		            
		            <tr class="form-textarea">
					    <th><label for="diastaz">Есть ли диастаз (расхождение передней мышцы живота после беременности) ?</label></th>
					    <td><textarea name="diastaz" id="diastaz" rows="5" cols="1"><?php the_author_meta( 'diastaz', $current_user->ID ); ?></textarea></td>
		            </tr>
		
					<tr class="form-textarea">
					    <th><label for="thyroid">Есть ли проблемы с щитовидной железой?</label></th>
					    <td><textarea name="thyroid" id="thyroid" rows="5" cols="1"><?php the_author_meta( 'thyroid', $current_user->ID ); ?></textarea></td>
		            </tr>
					...
					<table class="lk-table-food">
						...
						<tr class="form-textarea">
							...
							<td><textarea name="vitamins" id="vitamins" rows="5" cols="1"><?php the_author_meta( 'vitamins', $current_user->ID ); ?></textarea></td>
						</tr>
							
						<tr class="form-textarea">
			                <th><label for="medicines">Принимаете ли лекарства на постоянной основе?</label></th>
			                <td><textarea name="medicines" id="medicines" rows="5" cols="1"><?php the_author_meta( 'medicines', $current_user->ID ); ?></textarea></td>
            			</tr>

						<tr class="form-textarea">
			                <th><label for="day_menu">Опишите свое меню за день</label></th>
			                <td><textarea name="day_menu" id="day_menu" rows="5" cols="1"><?php the_author_meta( 'day_menu', $current_user->ID ); ?></textarea></td>
            			</tr>

						<tr class="form-textarea">
			                <th><label for="bad_food_for_you">Есть ли пищевая аллергия или непереносимость?</label></th>
			                <td><textarea name="bad_food_for_you" id="bad_food_for_you" rows="5" cols="1"><?php the_author_meta( 'bad_food_for_you', $current_user->ID ); ?></textarea></td>
            			</tr>
            
						<tr class="form-textarea">
			                <th><label for="milk_food">Как относитесь к молочным продуктам?</label></th>
			                <td><textarea name="milk_food" id="milk_food" rows="5" cols="1"><?php the_author_meta( 'milk_food', $current_user->ID ); ?></textarea></td>
            			</tr>
        			</table>
					...
					<table class="lk-table-workout">
						...
						<tr class="form-textarea">
			                ...
			                <td><textarea name="diet" id="diet" rows="5" cols="1"><?php the_author_meta( 'diet', $current_user->ID ); ?></textarea></td>
            			</tr>
            
						<tr class="form-textarea">
							<th><label for="cardio">Есть ли дома кардиотренажер?</label></th>
							<td><textarea name="cardio" id="cardio" rows="5" cols="1"><?php the_author_meta( 'cardio', $current_user->ID ); ?></textarea></td>
						</tr>

						<tr class="form-textarea">
			                <th><label for="workout_experience">Опыт тренировок</label></th>
			                <td><textarea name="workout_experience" id="workout_experience" rows="5" cols="1"><?php the_author_meta( 'workout_experience', $current_user->ID ); ?></textarea></td>
            			</tr>
            
						<tr class="form-textarea">
			                <th><label for="sport_last_time">Когда занимались спортом последний раз?</label></th>
			                <td><textarea name="sport_last_time" id="sport_last_time" rows="5" cols="1"><?php the_author_meta( 'sport_last_time', $current_user->ID ); ?></textarea></td>
            			</tr>
        	...
        	<div class="lk-menu-buy" style="<?php echo $lk_menu_buy_div_flag ?>">		
					...
					?>
							...
							<tr class="form-age">
		                        ...
		                        <td><input class="text-input" name="men_menu_age" type="number" id="men_menu_age" style="width: 100%;" value="<?php the_author_meta( "men_menu_age", $current_user->ID ) ?>" /></td>
		     
		                    </tr>              
		                    
		                    <tr class="form-height">
		                        <th><label for="men_menu_height">Рост мужчины, до 1см</label></th>
		                        <td><input class="text-input" name="men_menu_height" type="number" id="men_menu_height" style="width: 100%;" value="<?php the_author_meta( "men_menu_height", $current_user->ID ) ?>" /></td>
		                 
		                    </tr>
		                    
		                     <tr class="form-weight-at-start">
		                        <th><label for="men_menu_weight_at_start">Текущий вес мужчины, до 0,1кг</label></th>
		                        <td><input class="text-input" name="men_menu_weight_at_start" type="number" id="men_menu_weight_at_start" step="0.1" style="width: 100%;" value="<?php the_author_meta( "men_menu_weight_at_start", $current_user->ID ) ?>" /></td>			
		                    </tr>
		                    ...
		                    <tr id="daily-activity-row">
		                        ...
		                        <td>
			            <?php    
			                       	$men_menu_daily_activity_value = get_user_meta( $current_user->ID, 'men_menu_daily_activity', true);
		                ...
		                ?>    
		                    <tr class="form-textarea">
					                ...
					                <td><textarea name="men_menu_health_problems" id="men_menu_health_problems" rows="5" cols="1"><?php the_author_meta( 'men_menu_health_problems', $current_user->ID ) ?></textarea></td>
		            		</tr>
		            		
		            		<tr class="form-textarea">
					                <th><label for="men_menu_diet">Какие диеты у вас были до сегодняшнего дня?</label></th>
					                <td><textarea name="men_menu_diet" id="men_menu_diet" rows="5" cols="1"><?php the_author_meta( 'men_menu_diet', $current_user->ID ) ?></textarea></td>
		            		</tr>
		            		
		            		<tr class="form-textarea">
					                <th><label for="men_menu_workout">Есть ли физическая активность, тренировки? Какие и как часто?</label></th>
					                <td><textarea name="men_menu_workout" id="men_menu_workout" rows="5" cols="1"><?php the_author_meta( 'men_menu_workout', $current_user->ID ) ?></textarea></td>
		            		</tr>
            
							<tr class="form-textarea">
				                <th><label for="men_menu_what_result">Какой результат хотите достичь?</label></th>
				                <td><textarea name="men_menu_what_result" id="men_menu_what_result" rows="5" cols="1"><?php the_author_meta( 'men_menu_what_result', $current_user->ID ) ?></textarea></td>
					...
					?>
							...
							<tr class="form-age">
		                        ...
		                        <td><input class="text-input" name="women_menu_age" type="number" id="women_menu_age" style="width: 100%;" value="<?php the_author_meta( "women_menu_age", $current_user->ID ) ?>" /></td>
		     
		                    </tr>              
		                    
		                    <tr class="form-height">
		                        <th><label for="women_menu_height">Рост женщины, до 1см</label></th>
		                        <td><input class="text-input" name="women_menu_height" type="number" id="women_menu_height" style="width: 100%;" value="<?php the_author_meta( "women_menu_height", $current_user->ID ) ?>" /></td>
		                 
		                    </tr>
		                    
		                     <tr class="form-weight-at-start">
		                        <th><label for="men_menu_weight_at_start">Текущий вес женщины, до 0,1кг</label></th>
		                        <td><input class="text-input" name="women_menu_weight_at_start" type="number" id="women_menu_weight_at_start" step="0.1" style="width: 100%;" value="<?php the_author_meta( "women_menu_weight_at_start", $current_user->ID ) ?>" /></td>			
		                    </tr>
		                    ...
		                    <tr id="daily-activity-row">
		                        ...
		                        <td>
			            <?php    
			                       	$women_menu_daily_activity_value = get_user_meta( $current_user->ID, 'women_menu_daily_activity', true);
		                ...
		                ?>  
							...
							<th><label for="women_menu_breastfeed">Грудное вскармливание на сегодня</label></th>
								<td>
								<?php 
		                       		$women_menu_breastfeed_value = get_user_meta ( $current_user->ID, 'women_menu_breastfeed', true);
		                    ...
		                    <tr class="form-textarea">
					                ...
					                <td><textarea name="women_menu_health_problems" id="women_menu_health_problems" rows="5" cols="1"><?php the_author_meta( 'women_menu_health_problems', $current_user->ID ) ?></textarea></td>
		            		</tr>
		            		
		            		<tr class="form-textarea">
					                <th><label for="women_menu_diet">Какие диеты у вас были до сегодняшнего дня?</label></th>
					                <td><textarea name="women_menu_diet" id="women_menu_diet" rows="5" cols="1"><?php the_author_meta( 'women_menu_diet', $current_user->ID ) ?></textarea></td>
		            		</tr>
		            		
		            		<tr class="form-textarea">
					                <th><label for="women_menu_workout">Есть ли физическая активность, тренировки? Какие и как часто?</label></th>
					                <td><textarea name="women_menu_workout" id="women_menu_workout" rows="5" cols="1"><?php the_author_meta( 'women_menu_workout', $current_user->ID ) ?></textarea></td>
		            		</tr>
            
							<tr class="form-textarea">
				                <th><label for="men_menu_what_result">Какой результат хотите достичь?</label></th>
				                <td><textarea name="women_menu_what_result" id="women_menu_what_result" rows="5" cols="1"><?php the_author_meta( 'women_menu_what_result', $current_user->ID ) ?></textarea></td>
...
<?php endif; ?>				
        ...
        <?php
	    //}; //окончание блокировки пользователя по ID
	    ?>
